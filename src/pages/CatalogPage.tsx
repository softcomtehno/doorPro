  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-[#3A2618] mb-6">
        {filters.type === 'interior' ? 'Межкомнатные двери' :
         filters.type === 'entrance' ? 'Входные двери' :
         filters.type === 'sliding' ? 'Раздвижные двери' :
         filters.type === 'glass' ? 'Стеклянные двери' :
         filters.sale ? 'Акции и спецпредложения' :
         filters.search ? `Поиск: ${filters.search}` : 'Каталог дверей'}
      </h1>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        `}>
          ...
          {(Object.keys(filters).length > 0 && !(Object.keys(filters).length === 1 && (filters.type || filters.search))) && (
            <button
        ...
        <div className="lg:w-3/4">
          ...
          {products.length > 0 ? (
            ...
            <div className="bg-white rounded-lg shadow-md p-8 text-center">
              <h3 className="text-xl font-medium text-[#3A2618] mb-2">Товары не найдены</h3>
