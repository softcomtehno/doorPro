...
import { Heart, BarChart2, ShoppingCart, Truck, Shield, ArrowLeft, ChevronRight, Plus, Minus, Check } from 'lucide-react';
  ...
  if (!product) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <Link to="/catalog" className="mt-4 inline-block text-[#D4AF37] hover:text-[#3A2618] transition-colors">
          Вернуться в каталог
        </Link>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <nav className="flex mb-6 text-sm">
        ...
        <Link to="/catalog" className="text-gray-500 hover:text-[#D4AF37] transition-colors">Каталог</Link>
        <ChevronRight size={14} className="mx-2 text-gray-400" />
        <span className="text-gray-700 truncate max-w-[200px]">{product.name}</span>
      </nav>
      
      {/* Back to catalog */}
      <Link to="/catalog" className="inline-flex items-center text-[#3A2618] hover:text-[#D4AF37] transition-colors mb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <div className="flex space-x-2">
            {product.images.map((image, index) => (
              <button 
                ...
                className={`rounded-md overflow-hidden border-2 ${activeImage === index ? 'border-[#D4AF37]' : 'border-transparent'}`}
              >
                <img 
                  src={image} 
        ...
        <div>
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <h1 className="text-3xl font-bold text-[#3A2618] mb-2">{product.name}</h1>
            ...
            <div className="flex items-center mb-4">
              <div className="flex items-center">
                ...
                <span className="ml-1 text-gray-500">({product.reviewCount} отзывов)</span>
              </div>
              ...
              <div className="ml-auto flex space-x-2">
                ...
                >
                  <Heart size={20} className="text-gray-500 hover:text-[#D4AF37]" />
                </button>
                <button 
                  className={`p-2 rounded-full transition-colors ${isInComparison(product.id) ? 'bg-[#D4AF37] text-white' : 'hover:bg-gray-100'}`}
                  onClick={() => addToComparison(product)}
                  title="Добавить к сравнению"
                >
                  <BarChart2 size={20} className={isInComparison(product.id) ? 'text-white' : 'text-gray-500 hover:text-[#D4AF37]'} />
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center mb-6">
              ...
              <span className={`ml-4 px-3 py-1 rounded-full text-sm ${product.inStock ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'}`}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.options.colors.map((color) => (
                      <button
                        key={color}
                        className={`px-3 py-1 border rounded-full transition-colors ${selectedOptions.color === color ? 'border-[#D4AF37] bg-[#D4AF37] text-white' : 'border-gray-300 hover:border-[#D4AF37]'}`}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {product.options.handles.map((handle) => (
                      <label key={handle.name} className="flex items-center cursor-pointer group">
                        <div className={`
                          w-5 h-5 flex items-center justify-center mr-2 border rounded-full
                          ${selectedOptions.handle?.name === handle.name ? 'bg-[#D4AF37] border-[#D4AF37]' : 'border-gray-300 group-hover:border-[#D4AF37]'}
                          transition-colors
                        `}>
                          {selectedOptions.handle?.name === handle.name && <Check size={12} className="text-white" />}
                        </div>
                        <span className="text-gray-700 mr-2">{handle.name}</span>
                        {handle.price > 0 && (
                          <span className="text-sm text-gray-500">+{handle.price.toLocaleString()} ₽</span>
                        )}
                      </label>
                    ))}
                  </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Доставка по Москве: от 1000 ₽</p>
                </div>
              </div>
              <div className="flex items-start">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-12">
        <h2 className="text-2xl font-bold text-[#3A2618] mb-6">Характеристики</h2>
            ...
            <table className="w-full">
              <tbody>
                ...
                <tr className="border-b border-gray-200">
                  ...
                  <td className="py-3 font-medium">{product.features.height}</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table className="w-full">
              <tbody>
                <tr className="border-b border-gray-200">
                  ...
                  <td className="py-3 font-medium">{product.features.thickness}</td>
                </tr>
                {product.features.soundproofing && (
                  <tr className="border-b border-gray-200">
                    <td className="py-3 text-gray-600">Шумоизоляция, дБ</td>
                    <td className="py-3 font-medium">{product.features.soundproofing}</td>
                  </tr>
                )}
                {product.features.heatInsulation && (
                  <tr className="border-b border-gray-200">
                    <td className="py-3 text-gray-600">Теплоизоляция, м²•°C/Вт</td>
                    <td className="py-3 font-medium">{product.features.heatInsulation}</td>
                  </tr>
                )}
                {product.features.lockType && (
                  <tr className="border-b border-gray-200">
                    <td className="py-3 text-gray-600">Тип замка</td>
                    <td className="py-3 font-medium">{product.features.lockType}</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      {relatedProducts.length > 0 && (
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-[#3A2618] mb-6">Похожие товары</h2>
